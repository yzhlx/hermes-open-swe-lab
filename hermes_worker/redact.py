...
_GITHUB_PAT_RE = re.compile(r"\b(gh[opushr])_[A-Za-z0-9]{36}\b")
...
def redact(text: str) -> str:
    ...
    # 4) GitHub PAT prefixes
    text = _GITHUB_PAT_RE.sub(lambda m: m.group(1) + "_***REDACTED***", text)
...
def redact_secret_env_value(key: str, value: str) -> str:
    """Redact a single env value if its key is a known secret env var."""
